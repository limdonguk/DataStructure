...
LinkedList *createLinkedList(){
    ...
    if(pReturn != NULL) {                                              // NULL 이 아닐 경우에만 memset
...
int addLinkedListData(LinkedList* pList, int position, int data) {
    if(pList != NULL) { // 리스트 점검
        if(position > 0 && position <= pList->currentCount) {  // 입력 Parameter의 유효값 점검
            int i = 0;
...
void concatLinkedList(LinkedList *pListA, LinkedList *pListB) {
    ...
    if(pListA != NULL && pListB != NULL) {
        ...
        if(pNodeA != NULL) {
            while(pNodeA->pLink != NULL) { // pListA 자체는 존재하지만 data가 없을 경우
                pNodeA = pNodeA->pLink;
            }
